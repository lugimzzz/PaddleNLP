...
def quick_lora(
    ...
    if is_column:
        ...
        return ColumnQuickLora.apply(
            ...
            scaling,
            group,
            input_stop_gradient=input_stop_gradient,
        )
    elif is_row:
...
class QuickLora(PyLayer):
    ...
    def forward(
        ...
        ctx.quant_algo = quant_algo
        ctx._dtype = _dtype
        weight = weight_dequantize(quant_weight, quant_scale, ctx.quant_algo, ctx._dtype)
        merged_weight = paddle.addmm(weight, lora_A, lora_B, beta=1.0, alpha=scaling)
        ctx.input_stop_gradient = input_stop_gradient
        ctx.scaling = scaling
        ctx.save_for_backward(input, lora_A, lora_B, quant_weight, quant_scale)
        result = linear_func(input, merged_weight, bias)
        return result

    @staticmethod
    def backward(ctx, grad_output):
        input, lora_A, lora_B, quant_weight, quant_scale = ctx.saved_tensor()
